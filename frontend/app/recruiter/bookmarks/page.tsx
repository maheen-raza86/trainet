...
import { BookmarkIcon, ChatBubbleLeftRightIcon, ArrowRightIcon } from '@heroicons/react/24/outline';

interface Bookmark {
  id: string;
  created_at: string;
  profiles: { id: string; first_name: string; last_name: string; email: string; skills: string; bio: string };
}
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="text-center py-16 bg-white/60 rounded-2xl border border-white/30">
            ...
            <button onClick={() => router.push('/recruiter/talent')} className="px-6 py-3 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-xl hover:from-purple-600 hover:to-blue-600 transition">Browse Talent Pool</button>
          </div>
        ) : (
          bookmarks.map(b => {
            const p = b.profiles;
              ...
              <div key={b.id} className="bg-white/60 backdrop-blur-sm rounded-2xl border border-white/30 p-5 hover:bg-white/80 transition">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => router.push(`/recruiter/candidate/${p.id}`)}
                      className="p-2 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition" title="View profile">
                      <ArrowRightIcon className="w-4 h-4" />
                    </button>
                    <button onClick={() => router.push(`/recruiter/messages/${p.id}`)}
                    ...
                    <button onClick={() => removeBookmark(p.id)}
                      className="p-2 border border-red-200 text-red-500 rounded-xl hover:bg-red-50 transition" title="Remove bookmark">
