...
export default function RecruiterConversationPage() {
  ...
  useEffect(() => { if (user && userId) fetchMessages(); }, [user, userId]);
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);

  const fetchMessages = async () => {
    try {
      const res: any = await apiClient.get(`/recruiter/messages/${userId}`);
  ...
  const handleSend = async () => {
    ...
    try {
      setSending(true);
      await apiClient.post('/recruiter/message', { receiverId: userId, message: text.trim() });
      setText('');
      fetchMessages();
    } catch (err: any) {
  ...
  return (
    <DashboardLayout title={partnerName || 'Conversation'} subtitle="Direct message">
      <div className="flex flex-col h-[60vh] bg-white/60 backdrop-blur-sm rounded-2xl border border-white/30 overflow-hidden max-w-2xl">
